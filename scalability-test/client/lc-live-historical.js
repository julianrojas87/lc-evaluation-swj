...
async function getURLSet() {
    return JSON.parse(await readFile(path.join(process.cwd(), 'query-sets', 'nmbs-live-historic', 'urls.json'), 'utf8'));
}

async function getQuerySet() {
    return JSON.parse(await readFile(path.join(process.cwd(), 'query-sets', 'nmbs-live-historic', 'query-set.json'), 'utf8'));
}
